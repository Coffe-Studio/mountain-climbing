...
@export_group("Visual")
@export var panel_width: float = 270.0 # Reduzido de 660 para caber em 320px
...
@export_range(0.0, 100.0) var default_master_volume: float = 100.0
@export_range(0.0, 100.0) var default_music_volume: float = 80.0
@export_range(0.0, 100.0) var default_sfx_volume: float = 80.0
...
var language_option_button: OptionButton
...
func build_ui() -> void:
	...
	# Container de Margem interna para evitar colar nas bordas da tela
...
# ⚙
	...
	# Spacer flexível para empurrar o botão de fechar para o fundo
...
func show_general_page() -> void:
	...
	content.add_child(create_subtitle("Interface language"))
	...
	language_option_button = OptionButton.new()
	language_option_button.custom_minimum_size.x = 90
	language_option_button.custom_minimum_size.y = 18
	language_option_button.add_theme_font_size_override("font_size", 9)
	...
	if loc_mgr != null:
		...
		for code in loc_mgr.LANGUAGES.keys():
			var lang_name: String = loc_mgr.LANGUAGES[code]
			language_option_button.add_item(lang_name, idx)
			language_option_button.set_item_metadata(idx, code)

			if code == loc_mgr.current_language:
				selected_idx = idx
			idx += 1

		language_option_button.select(selected_idx)

	language_option_button.item_selected.connect(_on_language_selected)
	row.add_child(language_option_button)
...
func _on_language_selected(index: int) -> void:
	if language_option_button:
		var lang_code: String = language_option_button.get_item_metadata(index)
		var loc_mgr = get_node_or_null("/root/LocalizationManager")
		if loc_mgr and loc_mgr.has_method("change_language"):
			loc_mgr.change_language(lang_code)
		save_settings()
...
func show_controls_page() -> void:
	...
	for i in action_names.size():
		...
		var label_text: String = action_labels[i] if i < action_labels.size() else action
		...
		button.pressed.connect(func(): start_remap(action, button))
...
func show_audio_page() -> void:
	...
	master_slider = create_volume_slider("Master", default_master_volume)
	music_slider = create_volume_slider("Music", default_music_volume)
	sfx_slider = create_volume_slider("SFX", default_sfx_volume)
...
func create_volume_slider(label_text: String, default_value: float) -> HSlider:
...
func apply_and_save_audio() -> void:
	apply_bus_volume(master_bus_name, master_slider.value)
	apply_bus_volume(music_bus_name, music_slider.value)
	apply_bus_volume(sfx_bus_name, sfx_slider.value)
	save_settings()
...
func apply_audio_settings() -> void:
	...
	if err == OK:
		master_value = float(config.get_value("audio", "master", default_master_volume))
		music_value = float(config.get_value("audio", "music", default_music_volume))
		sfx_value = float(config.get_value("audio", "sfx", default_sfx_volume))

	apply_bus_volume(master_bus_name, master_value)
	apply_bus_volume(music_bus_name, music_value)
	apply_bus_volume(sfx_bus_name, sfx_value)
...
func load_settings_to_sliders() -> void:
	...
	if master_slider:
		master_slider.value = float(config.get_value("audio", "master", default_master_volume))
	if music_slider:
		music_slider.value = float(config.get_value("audio", "music", default_music_volume))
	if sfx_slider:
		sfx_slider.value = float(config.get_value("audio", "sfx", default_sfx_volume))


func reset_audio_to_default() -> void:
	if master_slider: master_slider.value = default_master_volume
	if music_slider: music_slider.value = default_music_volume
	if sfx_slider: sfx_slider.value = default_sfx_volume
...
func start_remap(action: String, button: Button) -> void:
	waiting_action = action
	waiting_button = button
	button.text = "Press key..."


func remap_action(action: String, keycode: Key) -> void:
...
func reset_controls_to_default() -> void:
	for i in action_names.size():
		if i < default_keys.size():
			remap_action(action_names[i], default_keys[i])
	save_settings()
...
func save_settings() -> void:
	...
	if language_option_button != null and language_option_button.selected >= 0:
		var lang_code: String = language_option_button.get_item_metadata(language_option_button.selected)
		config.set_value("general", "language", lang_code)

	if master_slider: config.set_value("audio", "master", master_slider.value)
	if music_slider: config.set_value("audio", "music", music_slider.value)
	...
	for action_variant in action_names:
		...
		for event in InputMap.action_get_events(action):
			if event is InputEventKey:
				config.set_value("keys", action, int((event as InputEventKey).keycode))
				break
...
func load_settings() -> void:
	...
	var loc_mgr = get_node_or_null("/root/LocalizationManager")
	if loc_mgr and loc_mgr.has_method("change_language"):
		var saved_lang: String = String(config.get_value("general", "language", "en"))
	...
	for action_variant in action_names:
		...
		if config.has_section_key("keys", action):
			var keycode: Key = int(config.get_value("keys", action))
			InputMap.action_erase_events(action)
			var ev: InputEventKey = InputEventKey.new()
			ev.keycode = keycode
			InputMap.action_add_event(action, ev)
...
func create_title(text: String) -> Label:
	var label: Label = Label.new()
	label.text = text
	label.add_theme_font_size_override("font_size", 13) # Ajustado para 320x180
	return label
...
func create_subtitle(text: String) -> Label:
	var label: Label = Label.new()
	label.text = text
	label.add_theme_font_size_override("font_size", 9) # Ajustado para 320x180
	return label


func create_separator() -> HSeparator:
	var sep: HSeparator = HSeparator.new()
	sep.custom_minimum_size.y = 4 # Reduzido de 16 para 4
	return sep
...
func open_settings() -> void:
	...
	tween.tween_property(root_panel, "position:x", 0.0, animation_time)
...
func close_settings() -> void:
	...
	tween.tween_property(root_panel, "position:x", -panel_width, animation_time)
